...
namespace WhitecatIndustries.Source
{
    ...
    public class NBodyConicsManager : MonoBehaviour
    {
        ...
        public static void ManagePredictions(Vessel vessel)
        {
            ...
            {
                InitialPosition = VesselPointsAtTime[VesselPointsAtTime.Count-1].Key;
            ...
            {
                InitialVelocity = VesselVelocitiesAtTime[VesselVelocitiesAtTime.Count-1].Key;
        ...
        public static void RenderTrajectory()
        {
                ...
                for (int i = 0; i < VesselPointsAtTime.Count; i++, i++, i++) // For every three (for every lerp count)
                {
                    ...
                    {
                        //Start Point 
                        Vector3d StartPoint = VesselPointsAtTime[i-1].Key;
                        // End Point
                        Vector3d EndPoint = VesselPointsAtTime[i].Key;
                        ...
                        lineRenderer.SetPosition((i), PlanetariumCamera.Camera.WorldToScreenPoint((ScaledSpace.LocalToScaledSpace(VesselPointsAtTime[i].Key))));
